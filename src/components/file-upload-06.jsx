...
export default function FileUpload06({ value, onChange }) {
  ...
  const onFileInputChange = (event) => {
    ...
    addFiles([file]); // لإظهار الـ preview

    onChange(file); // لإرسال الملف إلى react-hook-form
  };
                ...
                <div key={file.id} className="group flex items-center py-4">
                  <div className="mr-3 grid size-10 shrink-0 place-content-center rounded border bg-muted">
                    <Image
                      ...
                      width={50}
                      height={50}
                      className="h-full w-full rounded object-cover group-hover:hidden"
                    />
                    <Button
                      variant="ghost"
                      size="icon"
